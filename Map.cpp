...
void Map::resetMap()
{
    ...
    for (int i = 0; i < num_rows_; i++)
    {
        ...
        {
            map_data_[i][j] = UNEXPLORED; //marking each room as unexplored 
        }
    }
 ...
 * 1. return true if exit
 * 2. return false if not an exit 
 */
...
/**
 * Algorithm: add rooms and NPCs to map 
 * 1. initialize a counter to 0
 * 2. while the counter is less than five, call the addRoom function with random numbers between 0 and 11 
 * 3. if the addRoom function returns true, meaning the room was added to a non occupied space, the counter will be incremented 
 * 4. reset the counter to 0
 * 5. while the counter is less than five, call the addNPC function with random numbers between 0 and 11 
 * 6. if the addNPC function returns true, meaning the NPC was added to a non occupied space, the counter will be incremented 
 */

void Map::setupMap(Random &random) {

    // add 5 rooms randomly
    int counter = 0;
    while(counter < 5) {
        if(addRoom(random.get(0,11),random.get(0,11))) {
            counter++;
        }
    }
    ...
    counter = 0;
    while(counter < 5) {
        if(addNPC(random.get(0,11),random.get(0,11))) {
            counter++;
        }
    }
}
